...
public: // methods
	...
	unsigned int SearchStep(Map* map) {
		...
		} else // not goal
		{
			...
			bool ret = n->m_UserState.getSuccessors(this, map, n->parent ? &n->parent->m_UserState : NULL);
			...
			for (typename vector< Node * >::iterator successor = m_Successors.begin(); successor != m_Successors.end(); successor++) {
				...
				double newg = n->g + n->m_UserState.getCost((*successor)->m_UserState, map);
	...
	// Returns DBL_MAX if goal is not defined or there is no solution
	...
	UserState *GetOpenListStart(double &f, double &g, double &h) {
	...
	UserState *GetOpenListNext(double &f, double &g, double &h) {
		iterDbgOpen++;
		if (iterDbgOpen != m_OpenList.end()) {
			f = (*iterDbgOpen)->f;
			g = (*iterDbgOpen)->g;
	...
	UserState *GetClosedListStart(double &f, double &g, double &h) {
	...
	UserState *GetClosedListNext(double &f, double &g, double &h) {
		iterDbgClosed++;
		if (iterDbgClosed != m_ClosedList.end()) {
			f = (*iterDbgClosed)->f;
			g = (*iterDbgClosed)->g;
...
public:
	virtual ~AStarState() { }
	virtual double GoalDistanceEstimate(T &nodeGoal) = 0; // Heuristic function which computes the estimated cost to the goal node
	...
	virtual double GetCost(T &successor) = 0; // Computes the cost of travelling from this node to the successor node
